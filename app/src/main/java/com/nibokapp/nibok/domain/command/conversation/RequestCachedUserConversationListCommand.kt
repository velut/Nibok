...
/**
 * Request a cached list of ConversationModel instances that make up the conversations in which
 * the local user is participating.
 */
class RequestCachedUserConversationListCommand(
    ...
    override fun execute(): List<ConversationModel> =
            dataMapper.convertConversationListToDomain(
                    conversationRepository.getConversationList(cached = true)
            )
}